...
const Mapping=(prop)=>{
  ...
  useEffect(()=>{
    ...
    const grabDataFromAPI= async()=>{
      ...
      if(ipAddressRegex.test(userEnteredIP) || domainRegex.test(userEnteredIP)){
      var url=`https://geo.ipify.org/api/v2/country,city?apiKey=${process.env.REACT_APP_API_KEY}
      &
      ${ `ipAddress=${userEnteredIP}` || `domain=${userEnteredIP}`}`
    ...
    <>
      <div className="header">
        <h3>IP address Tracker</h3>
            ...
            <input 
             placeholder="Search for an IP or domian..."
             type="text" 
             value={userEnteredIP}
